...
module.exports = {
   ...
   execute(message, args, guildConf, serverQueue, queue, client) {
      ...
      if (args == undefined || args[0] == undefined){
         ...
         Object.keys(commandsCattegories)
         ...
         if (getCommandType(args[0]) == undefined){
            ...
            client.commands.filter(command => command.type == cattegory).forEach(command => {
               embed.addField(`${guildConf.prefix}${command.name}`, command.description)
            });
         }
